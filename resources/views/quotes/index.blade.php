                                        ...
                                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                            @foreach ($quotes as $key => $quote)
                                                <tr>
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200"> {{ $quote->company->name }} </td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200"> 
                                                        @if (strlen($quote->project->title) > 35)
                                                            {{ substr($quote->project->title, 0, 35) }}...
                                                        @else
                                                            {{ $quote->project->title }}
                                                        @endif
                                                    </td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200"> S/ {{ $quote->amount }} </td>
                                                    
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200"> <x-status-quotes :status="$quote->status" /> </td>
